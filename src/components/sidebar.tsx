...
const mainNavExtra = [
  { label: "Intelligence", tKey: "intelligencePage.title", href: "/dashboard/intelligence", icon: "M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z", badge: null, roles: ["super_admin", "admin", "manager", "agent"] },
];
...
const aiNav = [
  { label: "Agents", tKey: "dashboardAgentsPage.title", href: "/dashboard/agents", icon: "M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z", badge: null, roles: ["super_admin", "admin"] },
  { label: "Predictions", tKey: "predictionsPage.title", href: "/dashboard/predictions", icon: "M13 7h8m0 0v8m0-8l-8 8-4-4-6 6", badge: null, roles: ["super_admin", "admin"] },
  { label: "X-Ray Analysis", tKey: "xrayPage.title", href: "/dashboard/xray", icon: "M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z", badge: null, roles: ["super_admin", "admin"] },
];
...
const systemNav = [
  ...
  { label: "Payments", tKey: "dashboard.payments", href: "/payment", icon: "M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z", badge: null, roles: ["super_admin", "admin"] },
];
    ...
    <aside className="fixed inset-y-0 left-0 z-40 w-[268px] border-r border-gray-200/60 bg-white/80 backdrop-blur-2xl hidden lg:flex flex-col">
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto scrollbar-thin">
        ...
        <div className="mb-5">
          ...
          {filterByRole(mainNavExtra).map((item) => renderNavItem(item, "from-cyan-50 to-blue-50/80", "text-cyan-700", "text-cyan-600", "border-cyan-100/80", "bg-cyan-600"))}
        </div>
        ...
        {/* AI Section */}
        {filterByRole(aiNav).length > 0 && (
          <div className="mb-5">
            <div className="px-3.5 mb-2.5 text-[10px] font-bold text-gray-400/80 uppercase tracking-[0.15em]">AI</div>
            {filterByRole(aiNav).map((item) => renderNavItem(item, "from-pink-50 to-rose-50/80", "text-pink-700", "text-pink-600", "border-pink-100/80", "bg-pink-600"))}
          </div>
        )}
        ...
        <div>
          ...
          {filterByRole(systemNav).map((item) => renderNavItem(item, "from-amber-50 to-orange-50/80", "text-amber-700", "text-amber-600", "border-amber-100/80", "bg-amber-600"))}
        </div>
      </nav>
